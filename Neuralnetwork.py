...
class layer:
    def __init__(self, activation, activationder, inputam, outputam):
        ...
        self.weights = np.array([[random.random() / 5 - 1 / 10 for _ in range(inputam)] for _ in range(outputam)])
        self.biases = np.array([random.random() / 5 - 1 / 10 for _ in range(outputam)])
        self.input = []
        self.outputs = np.array([0 for i in range(len(self.biases))])
        self.prevouts = np.array([0 for i in range(len(self.biases))])
        self.previns = np.array([0 for i in range(inputam)])
        self.updatesw = np.array([[0 for _ in range(inputam)] for _ in range(outputam)])
        self.updatesb = np.array([0 for _ in range(outputam)])
        self.newcontribution = np.array([0 for i in range(len(self.weights[0]))])
    ...
    def calcupdate(self, contribution, alpha):
        ...
        for outindex in range(len(self.weights)):
            ...
            for inindex in range(len(self.weights[outindex])):
                ...
                self.newcontribution[inindex] = grad * self.weights[outindex][inindex] * self.previns[inindex] * contribution[outindex]
            self.updatesb[outindex] = grad * self.biases[outindex] * alpha * contribution[outindex]
        return self.newcontribution.copy()
